...
- ONLY use details, facts, memories, names, times, places, and specific information that appear in WHAT THEY SHARED above. Do not invent, assume, or add anything that was not explicitly provided. If a detail was not given, write around the absence — do not fill it in.

- Make it feel like it could only have been written by this specific person, about this specific person. Every sentence must be earned by what they shared — nothing generic, nothing assumed, nothing invented.`;
...
async function handleCollectiveOrder(session, customerEmail) {
  const metadata = session.metadata || {};
  const orderId = session.id; // use Stripe session ID as unique order ID

  const upstashUrl = process.env.UPSTASH_REDIS_REST_URL;
  const upstashToken = process.env.UPSTASH_REDIS_REST_TOKEN;

  // Store order data in Upstash
  const orderData = {
    orderId,
    email: customerEmail,
    ...
    occasionLabel: metadata.occasionLabel || '',
    createdAt: new Date().toISOString(),
  };

  await fetch(`${upstashUrl}/set/collective:${orderId}`, {
    method: 'POST',
    headers: {
  ...
  // Send organiser their contributor link
  const contributorLink = `https://lastwordletters.com/contribute?id=${orderId}`;

  const emailHtml = buildCollectiveEmailHTML({
  ...
  await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: customerEmail,
      subject: `Your contributor link is ready — One Voice, Many Hearts`,
      html: emailHtml,
    }),
  });
}
...
export default async function handler(req, res) {
  ...
  try {
    // Handle collective order differently — store and send link, don't generate letter yet
    if (packageId === 'collective') {
      console.log('Handling collective order for:', customerEmail);
      await handleCollectiveOrder(session, customerEmail);
      console.log('Collective order created, link sent to:', customerEmail);
      return res.status(200).json({ received: true, type: 'collective' });
    }

    // Standard letter generation for all other packages
...
function buildCollectiveEmailHTML({ organizerName, recipientName, contributorLink, orderId }) {
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"/><title>Your contributor link is ready</title></head>
  ...
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
      <tr><td style="text-align:center;padding-bottom:32px;">
        ...
        <div style="font-size:10px;letter-spacing:0.3em;text-transform:uppercase;color:#8a7a68;margin-top:4px;">One Voice, Many Hearts</div>
      </td></tr>
      <tr><td style="background:#fdfaf4;border:1px solid #e8e0ce;border-radius:3px;padding:44px 48px;">
        <p style="font-family:Georgia,serif;font-size:18px;font-style:italic;color:#4a3f32;margin:0 0 20px;">
          Dear ${organizerName},
        </p>
        <p style="font-size:15px;color:#4a3f32;line-height:1.85;margin:0 0 20px;">
          Your contributor link is ready. Share it with everyone who wants to add their voice to the letter for <strong>${recipientName}</strong>.
        </p>
        <table width="100%" cellpadding="0" cellspacing="0" style="margin:28px 0;">
          <tr><td style="background:#ede7d8;border:1px solid #d4c9b8;border-radius:3px;padding:16px 20px;">
            <div style="font-size:10px;letter-spacing:0.25em;text-transform:uppercase;color:#9a7a3a;margin-bottom:8px;">Your contributor link</div>
            <a href="${contributorLink}" style="font-size:14px;color:#9a7a3a;word-break:break-all;">${contributorLink}</a>
          </td></tr>
        </table>
        <p style="font-size:15px;color:#4a3f32;line-height:1.85;margin:0 0 14px;"><strong>What to do next:</strong></p>
        <ol style="font-size:14px;color:#4a3f32;line-height:1.9;margin:0 0 20px;padding-left:20px;">
          <li>Share the link above with your group</li>
          <li>Each person clicks the link and adds their memories</li>
          <li>When everyone is done, visit <a href="https://lastwordletters.com/one-voice-many-hearts?order_id=${orderId}" style="color:#9a7a3a;">your letter page</a> to generate the final letter</li>
          <li>The finished letter will be sent to this email address</li>
        </ol>
        <p style="font-size:14px;color:#8a7a68;font-style:italic;line-height:1.8;margin:0;">
          There's no deadline — generate the letter whenever you're ready.
        </p>
      </td></tr>
